...
# 값이 바뀌면 요약 한 줄로 알린다. 상태를 뺀 셋은 해시 대상이라 알림 트리거이기도 하다.
# 상태('마감' 배지)는 표시만 한다. 배지 문구가 어떻게 변하는지 아직 관찰이 부족해
# 해시에 넣으면 알림이 잦아질 수 있다. 추경공고가 뜨면 공고종류·접수기간·비고가
# 함께 바뀌어 어차피 알림이 나가고, 그때 이 줄이 상태 변화를 같이 보여준다.
_TEXT_FIELDS = [
    ("공고종류", "📌", "공고종류"),
    ("상태", "🚦", "상태"),
    ("접수기간", "📅", "접수기간"),
...
def format_daily_report(rows: list[dict], deltas: dict, now_str: str, url: str) -> str:
    ...
    for r in rows:
        ...
        parts.append("")
        상태 = r.get("상태", "")
        parts.append(f"🚙 <b>{_esc(차종)}</b>" + (f"  🚦 {_esc(상태)}" if 상태 else ""))
        parts.append(
